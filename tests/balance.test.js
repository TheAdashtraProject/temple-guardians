...
for(let map=0;map<5;map++)for(let encounter=0;encounter<3;encounter++){
 ...
 let safety=0;while(!['won','lost'].includes(b.status)&&safety++<30000){for(let i=0;i<order.length;i++)if(!b.shrines.some(s=>s.plot===positions[i]))b.build(positions[i],order[i]);if(b.shrines.length===7)for(const s of [...b.shrines].sort((a,z)=>a.level-z.level))b.upgrade(s.plot);if(b.status==='planning')b.startWave();for(const s of b.shrines.filter(s=>s.deity==='durga')){const e=b.enemies.filter(e=>b.visible(e)&&b.canAffect(e,'durga')&&(e.boon!=='iron'||e.stopped)).sort((a,z)=>Number(z.boss)-Number(a.boss)||z.hp-a.hp)[0];if(e)b.strike(s.plot,e.id);}if(b.meter>=100&&b.enemies.length>4){const s=b.shrines.find(s=>s.deity==='indra');if(s)b.intervention(s.plot);}if(b.enemies.length>5)b.useFeature();b.tick(.1);b.animate(.1);}
